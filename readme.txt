...
from dbmainfunc.scm
-dbmainfunc.scm contains the functions that represents the main features of the
program. This features use functions from dbfileutils.scm, dbparsers.scm, dbfunc.scm
...
-dbfunc.scm contains the auxiliar general functions that are used in the other files and are not
categorized in file management or parsing
...
it doesn`t show anymore that there were bad formed parameters. It just says that the file doesn't exist. 
It still says that there were bad formed parameters when the given file extension doesn't match the 
expected extension (.dat or .rep).
